    ...
    for epoch in range(init_epoch, args.num_epoch+1):
        ...
        with torch.no_grad():
            for i in range(len(val_data_t)):
                ...
                for j in range(val_data_t[i].shape[0] - args.num_channels + 1):
                    val_patch_t = torch.tensor(val_data_t[i][j:j + args.num_channels, :, :], device=device).unsqueeze(0)

                    x1_t = torch.cat((torch.randn_like(val_patch_t), val_patch_t), axis=1)
                    # diffusion steps
                    fake_sample1 = sample_from_model(pos_coeff, gen_diffusive_1, args.num_timesteps, x1_t, T, args)

                    val_ts[j:j + args.num_channels, :, :] += fake_sample1.cpu().detach().numpy()[0]
                    used[j:j + args.num_channels, :, :] += 1
