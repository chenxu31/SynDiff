...
def sample_and_test(args):
    ...
    with torch.no_grad():
        for i in range(len(test_data_t)):
            ...
            test_ts_mae[i] = abs(common_cmf.restore_hu(syn_im) - common_cmf.restore_hu(test_data_s[i])).mean()
            if args.output_path:
                common_cmf.save_nii(syn_im, "syn_ts_%d.nii.gz" % i)
        ...
        for i in range(len(test_data_s)):
            ...
            test_st_mae[i] = abs(common_cmf.restore_hu(syn_im) - common_cmf.restore_hu(test_data_t[i])).mean()
            if args.output_path:
